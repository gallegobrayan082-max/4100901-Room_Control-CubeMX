...
// Buffer para las teclas presionadas
uint8_t keypad_buffer[KEYPAD_BUFFER_LEN];

ring_buffer_t keypad_rb;
                            
// Estructura para el teclado
...
// Tiempo de la última tecla presionada (evitar rebote)
uint32_t last_key_time = 50;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) { 
    ...
    char key = keypad_scan(&keypad, GPIO_Pin);   // lee el teclado para obtener la tecla presionada
...
int main(void)
{
  ...
  // Inicia el LED, los buffers circulares y el teclado
  led_init(&led1);
  ring_buffer_init(&rb, ring_buffer, RING_BUFFER_SIZE);
  ring_buffer_init(&keypad_rb, keypad_buffer, KEYPAD_BUFFER_LEN);
  keypad_init(&keypad);

  printf("Introduzca clave de 4 dígitos:\r\n");

  char clave_correcta[5] = "B39C";
  ...
  while (1)
  {
    ...
    if (ring_buffer_read(&keypad_rb, &tecla)) { 
        ...
        if (idx < 4) {                     // Si aún no se han ingresado 4 dígitos
            ...
            HAL_Delay(100);
            led_off(&led1); // Apaga el LED después de un retardo
        }
        ...
        if (idx == 4) {                
            ...
            if (strcmp(clave_ingresada, clave_correcta) == 0) {   
                ...
                for (int i = 0; i < 3; i++) {
                    led_on(&led1);  // Enciende el LED para indicar acceso permitido
                    HAL_Delay(500);
